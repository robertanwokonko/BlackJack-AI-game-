...
%
appendScores(FinalTable) :-
	getScoreTable([D|Rest], FinalTable), % scores from the just concluded round
	nth1(1, Rest, P), % Gets player's score from the tail
	
	scores(DScore, PScore), % Get the last total scores

	NDScore is DScore + D, % New Dealer score
	NPScore is PScore + P, % New Dealer score

	write('--------------------------------------------------------------------------------'), nl, 
	
	( ( (D = P) -> write('Game is a tie!') ) % When game is a tie
	; (D = 1) -> write('Dealer wins!') % When dealer wins
	; otherwise -> write('You win!') % When player wins
	), nl,
	
	write('================================================================================'), nl,
	write('Dealer score: '), write(NDScore), nl,
	write('Player score: '), write(NPScore), nl, nl,

	retractall( scores(_, _) ),
	assertz( scores(NDScore, NPScore) ).
...
printHands([H|T], Total, N, Type) :-
	userPlayer(U),
	( N = 1                -> ( write('Dealer cards'), Restrict is 1 )
	; ( N = Total, U = 1 ) -> ( write('Your cards  '), Restrict is 0 )
